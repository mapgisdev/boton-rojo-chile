/**
 * =========================================================================================
 * BR-HR VS M0 ORIGINAL CONAF — VISOR DUAL COMPARATIVO A 5 DÍAS DE PRONÓSTICO (GEE)
 * =========================================================================================
 *
 * Versión Oficial: 2.1.0 — 100% Basada en Assets de Earth Engine y Ecuaciones Fieles
 * 
 * Insumos y Assets Oficiales:
 * - Límites Comunales:       projects/boton-rojo-chile/assets/comunas_chile
 * - Incendios Históricos:    projects/boton-rojo-chile/assets/incendios_conaf_historico
 * - Pronóstico Meteorológico: NOAA/GFS0P25 (5 días d0..d4) y ECMWF/ERA5_LAND/HOURLY (Histórico)
 * - Combustible:             ESA/WorldCover/v100 (Clases 10, 20, 30, 40, 90)
 * - Relieve Topográfico:     USGS/SRTMGL1_003 (Hillshade az=313°, alt=60°)
 * =========================================================================================
 */
...
// 2. Límites Oficiales de Chile y Assets
...
var coleccionComunas = ee.FeatureCollection(COMUNAS_ASSET);

var INCENDIOS_ASSET = "projects/boton-rojo-chile/assets/incendios_conaf_historico";
var coleccionIncendios = ee.FeatureCollection(INCENDIOS_ASSET);

// 3. Coberturas Estáticas: Combustible y Sombreado SRTM 90m
...
// Reclass G (M0 Oficial): <= 123.5 -> 200 (Sombreado), > 123.5 -> 100 (Expuesto)
var reclassG = hillshade.lte(123.5).multiply(200).add(hillshade.gt(123.5).multiply(100)).rename("reclassG");

// Matriz de Probabilidad de Ignición M0 CONAF (288 celdas oficiales)
var CLAVES_PI = [
...
leftMap.centerObject(ee.Geometry.Point([-72.5, -37.0]), 7);
...
sidebar.add(ui.Label({
  value: "Desplaza la cortina central para comparar el Botón Rojo original de CONAF frente al modelo continuo BR-HR a 5 días de pronóstico.",
  style: {fontSize: "11px", color: "#64748b", margin: "0 0 10px 0"}
}));

var selectorEscenarios = ui.Select({
  items: [
    {label: "🌐 Pronóstico Operativo (5 Días GFS + FIRMS)", value: "HOY"},
  ...
  onChange: function(val) {
    if (val === "HOY") {
      horizontePanel.style().set("shown", true);
      cargarPronosticoGFS(selectorHorizonte.getValue());
    } else {
      horizontePanel.style().set("shown", false);
      cargarEscenarioHistorico(val);
    }
  }
});
...
// Selector de Horizonte de Pronóstico (d0 a d4)
var selectorHorizonte = ui.Select({
  items: [
    {label: "📅 d0: Hoy (Día 1)", value: 0},
    {label: "📅 d1: +24 Horas (Día 2)", value: 1},
    {label: "📅 d2: +48 Horas (Día 3)", value: 2},
    {label: "📅 d3: +72 Horas (Día 4)", value: 3},
    {label: "📅 d4: +96 Horas (Día 5)", value: 4}
  ],
  value: 0,
  style: {width: "100%", margin: "2px 0 8px 0"},
  onChange: function(dayOffset) {
    cargarPronosticoGFS(dayOffset);
  }
});

var horizontePanel = ui.Panel({
  style: {margin: "0 0 6px 0", shown: false},
  widgets: [
    ui.Label({value: "⏱️ Horizonte de Pronóstico (5 Días):", style: {fontWeight: "bold", fontSize: "11px", color: "#1d3557"}}),
    selectorHorizonte
  ]
});
sidebar.add(horizontePanel);
...
// 6. CÁLCULO PARA ESCENARIOS HISTÓRICOS (ERA5-LAND)
function cargarEscenarioHistorico(fechaStr) {
  leftMap.layers().reset();
  rightMap.layers().reset();
  statusLabel.setValue("Procesando comparación M0 vs BR-HR para " + fechaStr + "...");

  var startDate = ee.Date(fechaStr);
  ...
  var tempC = era5Mean.select("temperature_2m").subtract(273.15).clip(chile).rename("temp_c");
  ...
  var rhPct = ee.Image(100).multiply(vp).divide(vps).clamp(3, 100).clip(chile).rename("rh_pct");

  var uWind = era5Mean.select("u_component_of_wind_10m");
  var vWind = era5Mean.select("v_component_of_wind_10m");
  var windKmh = uWind.hypot(vWind).multiply(3.6).clip(chile).rename("wind_kmh");

  procesarYRenderizar(tempC, rhPct, windKmh, "📅 " + fechaStr, startDate, endDate, true, fechaStr);
}

// 7. CÁLCULO PARA PRONÓSTICO OPERATIVO NOAA GFS (5 DÍAS d0..d4)
function cargarPronosticoGFS(dayOffset) {
  leftMap.layers().reset();
  rightMap.layers().reset();
  statusLabel.setValue("Consultando NOAA GFS 0.25° para día d" + dayOffset + "...");

  var gfs = ee.ImageCollection("NOAA/GFS0P25").filterBounds(chile);
  var latestImg = gfs.sort("system:time_start", false).first();
  var creationTime = latestImg.get("creation_time");

  var corrida = gfs.filter(ee.Filter.eq("creation_time", creationTime))
                   .filter(ee.Filter.gte("forecast_hours", 1))
                   .filter(ee.Filter.lte("forecast_hours", 120));

  var tagged = corrida.map(function(img) {
    var tUtc = ee.Date(img.get("forecast_time"));
    var mes = tUtc.get("month");
    var desfase = ee.Algorithms.If(mes.gte(5).and(mes.lte(8)), -4, -3);
    var tLocal = tUtc.advance(ee.Number(desfase), "hour");
    return img.set({
      "hora_local": tLocal.get("hour"),
      "fecha_local": tLocal.format("YYYY-MM-dd")
    });
  });

  var windowed = tagged.filter(ee.Filter.rangeContains("hora_local", 14, 18));
  var datesList = ee.List(windowed.aggregate_array("fecha_local")).distinct().sort().slice(0, 5);

  var targetDateStr = ee.String(datesList.get(dayOffset));

  var dayImgs = windowed.filter(ee.Filter.eq("fecha_local", targetDateStr));
  var dayMean = dayImgs.mean();

  var tempC = dayMean.select("temperature_2m_above_ground").subtract(273.15).clip(chile).rename("temp_c");
  var rhPct = dayMean.select("relative_humidity_2m_above_ground").clamp(3, 100).clip(chile).rename("rh_pct");
  var uGfs = dayMean.select("u_component_of_wind_10m_above_ground");
  var vGfs = dayMean.select("v_component_of_wind_10m_above_ground");
  var windKmh = uGfs.hypot(vGfs).multiply(3.6).clip(chile).rename("wind_kmh");

  var todayDate = ee.Date(Date.now());
  var targetDate = todayDate.advance(dayOffset, "day");

  targetDateStr.evaluate(function(dStr) {
    var lbl = "🌐 Pronóstico GFS (d" + dayOffset + ": " + dStr + ")";
    procesarYRenderizar(tempC, rhPct, windKmh, lbl, targetDate, targetDate.advance(1, "day"), false, dStr);
  });
}

// 8. MOTOR CIENTÍFICO COMÚN: EJECUTA M0 Y M1 SOBRE LAS MISMAS VARIABLES
function procesarYRenderizar(tempC, rhPct, windKmh, labelText, startDate, endDate, isHistorical, dateFilterStr) {

  // =========================================================================
  // A. MODELO ORIGINAL CONAF (LÍNEA BASE M0 — 100% FIEL)
  // =========================================================================
  ...
  // Reducción Comunal M0 (Regla Institucional >= 30% de Superficie Combustible)
  ...
  ee.Dictionary({
    ...
    n_fires: conafFC.size(),
    n_major: majorFires.size()
  }).evaluate(function(res) {
    var nr0 = res ? res.m0_red : 0;
    var nr1 = res ? res.m1_red : 0;
    var ny1 = res ? res.m1_yellow : 0;
    var nf = res ? res.n_fires : 0;
    var nm = res ? res.n_major : 0;

    statusLabel.setValue(labelText + " | M0: " + nr0 + " Rojas | BR-HR: " + nr1 + " Rojas, " + ny1 + " Amarillas" + (isHistorical ? (" | 🔥 " + nf + " focos") : ""));
    actualizarLeyendasComparativas(nr0, nr1, ny1, nf, nm, isHistorical);
  });
}

function actualizarLeyendasComparativas(nr0, nr1, ny1, nf, nm, isHistorical) {
  legendPanel.clear();
  ...
  legendPanel.add(ui.Label({
    value: "   (Regla rígida: T≥20, HR≤30, V≥20, HCFM≤8. Sin alerta preventiva)",
    style: {fontSize: "10px", color: "#64748b", margin: "0 0 4px 10px"}
  }));
  ...
  legendPanel.add(ui.Label({
    value: "   (Probabilidad continua PI: captura el corredor territorial completo)",
    style: {fontSize: "10px", color: "#64748b", margin: "0 0 6px 10px"}
  }));

  if (isHistorical && nf > 0) {
    legendPanel.add(ui.Label({
      value: "🔥 Evidencia Histórica CONAF: " + nf + " focos (" + nm + " ≥ 200 ha)",
...
cargarEscenarioHistorico("2023-02-03");
